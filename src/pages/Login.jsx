    ...
    <div className="flex flex-col items-center justify-between min-h-screen bg-[#0a0a0a]">
      <div className="flex flex-col items-center justify-center flex-1 w-full p-4">
        ...
        <div className="bg-gradient-to-br from-[#1a1a1a] to-[#0d0d0d] p-10 md:p-12 rounded-2xl border-2 border-primary shadow-[0_0_30px_rgba(255,107,0,0.3)] max-w-md w-full animate-[fadeIn_0.5s_ease-out]">
          ...
          <p className="text-gray-400 text-center mb-8 text-sm">Quality Control Tool</p>
          ...
          <div className="flex justify-between mt-6 text-sm">
            <button onClick={() => {setShowModal(true); setModalMessage({type: '', text: ''})}} className="text-gray-400 hover:text-primary transition-colors">
              Forgot Password?
            </button>
            <Link to="/change-password" className="text-gray-400 hover:text-primary transition-colors">
              Change Password
            </Link>
          </div>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 animate-[fadeIn_0.3s_ease]">
          <div className="bg-gradient-to-br from-[#1a1a1a] to-[#0d0d0d] p-8 rounded-2xl border-2 border-primary shadow-[0_0_30px_rgba(255,107,0,0.3)] max-w-sm w-[90%]">
            ...
            {modalMessage.text && (
              <div className={`p-3 rounded-lg mb-4 text-center text-sm font-medium ${modalMessage.type === 'error' ? 'text-status-danger bg-status-danger/10 border border-status-danger' : 'text-status-success bg-status-success/10 border border-status-success'}`}>
                {modalMessage.text}
              </div>
            )}
      ...
      {/* Using our new Reusable Lego Block! */}
      <Footer />
    </div>
  );
}