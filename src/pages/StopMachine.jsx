...
import { subscribeToActiveStoppedMachines, subscribeToMachineIssues, reportStoppedMachine, markIssueSolved, startMachine, addMachineIssue } from '../services/stoppedMachineOperations';
...
export default function StopMachine() {
  ...
  const handleSubmitIssues = async () => {
    ...
    if (newIssueText.trim()) {
      const result = await addMachineIssue(newIssueText.trim(), userFullName);
      if (result) issues.push({ id: result.id, label: result.label });
    }
    if (issues.length === 0) return;
    const result = await reportStoppedMachine(modalMachine, issues, userFullName);
    if (result === 'saved') {
    ...
    <Layout title="Report Stopped Machines" subtitle="Click a machine to report or resolve issues" maxWidth="max-w-6xl">
                ...
                <div className="mb-4">
                  ...
                  {currentRecord.issues?.length > 0 ? (
                    <div className="flex flex-col gap-2">
                      {currentRecord.issues.map(issue => {
                        const solved = !!issue.solvedAt;
                        return (
                          <div key={issue.id} className={`flex items-center justify-between p-3 rounded-lg border ${solved ? 'border-status-success/30 bg-status-success/10' : 'border-status-danger/30 bg-status-danger/10'}`}>
      ...
      {isReportModalOpen && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] animate-[fadeIn_0.3s_ease]" onClick={() => setIsReportModalOpen(false)}>
          <div className="bg-gradient-to-br from-[#1E1E1E] to-[#2d2d2d] p-8 rounded-2xl border-2 border-status-danger shadow-[0_20px_60px_rgba(244,67,54,0.3)] max-w-md w-[90%]" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto custom-scrollbar p-2 border border-[#333] rounded-lg">
            ...
            <div className="mb-5">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-primary text-black font-bold px-4 py-2 rounded-lg text-sm hover:bg-primary-dark transition-all"
                >
                  + Add
                </button>
              </div>
