...
import { fetchCartonRecordsByShift, getCartonWasteSummary } from '../services/cartonOperations';
import { buildShiftIdentifiers, calculateTrend } from '../services/reportUtils';

ChartJS.register(CategoryScale, LinearScale, BarElement, PointElement, LineElement, Title, Tooltip, Legend);

export default function CartonWasteReport() {
  ...
  const generateReport = useCallback(async () => {
    ...
    try {
      const allRecords = await fetchCartonRecordsByShift(config, shift, date);
      const machineSummary = await getCartonWasteSummary(config, shift, date);
      ...
      if (teamFilter !== 'all') {
        ...
        filteredSummary = machineSummary.map(m => {
          ...
          const totalAllocated = teamRecords.reduce((s, c) => s + c.allocated, 0);
          const totalUsed = teamRecords.reduce((s, c) => s + c.used, 0);
          const totalWasted = teamRecords.reduce((s, c) => s + c.wasted, 0);
  ...
  const totalWasted = records.reduce((s, r) => s + r.wasted, 0);
  const totalProcessed = totalUsed + totalWasted;
  const wastePercent = totalProcessed > 0 ? (totalWasted / totalProcessed) * 100 : 0;

  const targetWastePercent = config?.cartonWaste?.targetWastePercent ?? 5;
      ...
      tooltip: {
        callbacks: {
          afterLabel: function(context) {
            ...
            if (!m) return '';
            return `Total Used: ${m.totalUsed}\nTotal Wasted: ${m.totalWasted}`;
          }
        }
      }
    },
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Date', 'Shift', 'Team', 'Machine', 'Line', 'Round', 'Allocated', 'Used', 'Wasted', 'Waste %', 'CheckedBy', 'CheckedAt'];
    ...
    link.href = url;
    link.download = `carton_waste_${shift}_${date}.csv`;
    link.click();
  };

  if (loadingConfig) {
    return <Layout title="Carton Waste Report" subtitle="Loading..."><div className="text-center text-white mt-10 animate-pulse">Loading...</div></Layout>;
  }

  return (
    <Layout title="Carton Waste Report" subtitle="Analyze carton waste across shifts" maxWidth="max-w-7xl">
      <style>{`
        @media print {
      ...
      <div className="print:hidden">
        {!showFilters && (
          ...
          >
            <div className="text-xl font-bold text-primary mb-2 group-hover:text-white transition-colors">📦 Carton Waste Report</div>
            <div className="text-gray-400 text-sm">View waste analysis for any shift. Filter by team, machine, and compare across shifts.</div>
          </button>
        )}
      ...
      <div className="hidden print:block text-center mb-6">
        <h1 className="text-2xl font-black uppercase tracking-wider text-black">Carton Waste Report</h1>
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-500 py-20">
          ...
          <p className="text-sm text-gray-600">No carton waste records for {shift} {date}.</p>
        </div>
      ) : (
        <div className="animate-[fadeIn_0.5s_ease-out]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 print:grid-cols-4 print:gap-3">
            <div className="bg-dark-card p-5 rounded-xl border border-[#333] text-center">
              <div className="text-xs text-gray-400 uppercase font-bold tracking-wider mb-1">📦 Total Allocated</div>
              <div className="text-3xl font-bold text-primary">{totalAllocated.toLocaleString()}</div>
            </div>
            <div className="bg-dark-card p-5 rounded-xl border border-[#333] text-center">
              <div className="text-xs text-gray-400 uppercase font-bold tracking-wider mb-1">✅ Total Used</div>
              <div className="text-3xl font-bold text-white">{totalUsed.toLocaleString()}</div>
            </div>
            <div className="bg-dark-card p-5 rounded-xl border border-[#333] text-center">
              <div className="text-xs text-gray-400 uppercase font-bold tracking-wider mb-1">🗑️ Total Wasted</div>
              <div className="text-3xl font-bold text-status-warning">{totalWasted.toLocaleString()}</div>
            </div>
            <div className="bg-dark-card p-5 rounded-xl border border-[#333] text-center">
                ...
                <tbody className="divide-y divide-[#333]">
                  {crossShiftData.map((d, i) => (
                    <tr key={i} className={`hover:bg-white/5 ${d.isCurrent ? 'bg-primary/10' : ''}`}>
                      ...
                      <td className="p-3 text-gray-300">{d.totalAllocated?.toLocaleString() || '-'}</td>
                      <td className="p-3 text-gray-300">{d.totalUsed?.toLocaleString() || '-'}</td>
                      <td className="p-3 text-gray-300">{d.totalWasted?.toLocaleString() || '-'}</td>
              ...
              <tbody className="divide-y divide-[#333]">
                {summary.map(m => (
                  <tr key={m.machineId} className="hover:bg-white/5">
                    ...
                    <td className="p-3 text-gray-300">{m.totalAllocated}</td>
                    <td className="p-3 text-gray-300">{m.totalUsed}</td>
                    <td className="p-3 text-gray-300">{m.totalWasted}</td>
                ...
                <tr className="border-t-2 border-primary font-bold">
                  ...
                  <td className="p-3 text-white">{totalAllocated}</td>
                  <td className="p-3 text-white">{totalUsed}</td>
                  <td className="p-3 text-white">{totalWasted}</td>
              ...
              <tbody className="divide-y divide-[#333]">
                {records.sort((a, b) => {
                  ...
                  <tr key={r.id} className="hover:bg-white/5">
                    ...
                    <td className="p-3 text-gray-300">{r.allocated}</td>
                    <td className="p-3 text-gray-300">{r.remaining}</td>
                    <td className="p-3 text-gray-300">{r.used}</td>
                    <td className="p-3 text-gray-300">{r.wasted}</td>
...
async function buildCrossShiftData(config, currentShift, targetDateStr, teamFilter) {
  ...
  for (const s of shifts) {
    try {
      const records = await fetchCartonRecordsByShift(config, s.shift, s.date);
      ...
      if (filtered.length === 0) {
        data.push({ ...s, totalAllocated: 0, totalUsed: 0, totalWasted: 0, wastePercent: 0, hasData: false });
        continue;
      }

      const ta = filtered.reduce((sum, r) => sum + r.allocated, 0);
      const tu = filtered.reduce((sum, r) => sum + r.used, 0);
      const tw = filtered.reduce((sum, r) => sum + r.wasted, 0);
      const tp = tu + tw;
      const wp = tp > 0 ? (tw / tp) * 100 : 0;

      data.push({ ...s, totalAllocated: ta, totalUsed: tu, totalWasted: tw, wastePercent: Math.round(wp * 100) / 100, hasData: true });
    } catch (e) {
