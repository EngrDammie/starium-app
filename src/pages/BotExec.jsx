...
export default function BotExec() {
  ...
  useEffect(() => {
    ...
    const initializeData = async () => {
      const docId = await getOrCreateShiftApproval('bot', config);
      setApprovalId(docId);
      testsUnsubscribe = subscribeToShiftTests('bot', config, (tests) => setShiftTests(tests));
  ...
  const getDensityStatus = (density) => {
    ...
    if (d < config.botMinDensity) return 'low';
    if (d > config.botMaxDensity) return 'high';
    return 'normal';
  };
