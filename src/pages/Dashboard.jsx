...
export default function Dashboard() {
  ...
  const { currentUser, systemRole, departmentRoles } = useAuth();
  ...
  // Subscribe to real-time test counts for the dashboard
  useEffect(() => {
    if (loadingConfig) return;
    ...
    <Layout title="Factory Command Center" subtitle={`Live Overview • ${currentShift} Shift`} maxWidth="max-w-7xl">
      ...
      <div className="bg-gradient-to-r from-primary/20 to-transparent border-l-4 border-primary p-6 rounded-r-xl mb-8 animate-[fadeIn_0.5s_ease-out]">
        <h2 className="text-2xl font-bold text-white mb-1">
          Welcome back, <span className="text-primary">{currentUser?.email || 'Operator'}</span>
        </h2>
        <p className="text-gray-400 text-sm">
          System Role: <span className="uppercase font-bold text-white">{systemRole.replace('_', ' ')}</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        {/* Metric 3: Empty Silos (Coming Soon) */}
        ...
        {/* Metric 4: Stopped Machines (Coming Soon) */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-[fadeIn_1.1s_ease-out]">
        ...
        {/* Only show Data Entry if they have the role */}
